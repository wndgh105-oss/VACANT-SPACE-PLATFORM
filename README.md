...
**카카오맵 API는 실제로 연동돼 있습니다** (`NEXT_PUBLIC_KAKAO_MAP_KEY`). 상권 공공 API·PG·알림톡은 **연동하지 않았습니다.** 대체 구현은 아래 "가짜로 처리한 것" 참조.
...
| 알림 (이메일/SMS/알림톡) | 발송 없음 | 화면 내 인앱 알림 목록으로 대체 |
...
| `npm run build` | 통과 |
| `npm test` | 25개 파일 / 95개 테스트 통과 |
| `npm run test:e2e` | 6개 시나리오 통과 |
| 반응형 | 375px(모바일) / 1280px(데스크톱) 확인 |
| 데모 영상 | `docs/demo/demo.mp4` — 1분 15초 · H.264 1280×800 · 4.3MB · 재생 확인 (실사 승인·인앱 알림 장면 포함) |
